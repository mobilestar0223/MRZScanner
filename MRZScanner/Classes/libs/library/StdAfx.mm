...
Mat getMatImage(BYTE* pbyImg, int w, int h, int c)
{
	Mat matImg;
	if (c == 3)
		matImg = Mat(Size(w, h), CV_8UC3, pbyImg);
	else if (c == 1)
		matImg = Mat(Size(w, h), CV_8UC1, pbyImg);
	else if (c == 0)
	{
		matImg = Mat(Size(w, h), CV_8UC1);
		for (int i = 0; i < h; i++)
		{
			int pos1 = i * matImg.step;
			int pos2 = i * w;
			for (int j = 0; j < w; j++)
			{
				if (pbyImg[pos2 + j] == 0)
					matImg.data[pos1 + j] = 0xFF;
				else
					matImg.data[pos1 + j] = 0;
			}
		}
	}
...
void DisplayMatImage(Mat matImg, bool bResize)
{
	Mat matImgRe;
	if (bResize == true)
	{
		int h = 800;
		int w = h * matImg.size().width / matImg.size().height;
		resize(matImg, matImgRe, Size(w, h));
	}
	else
	{
		matImgRe = matImg.clone();
	}

	imshow("Image View", matImgRe);
